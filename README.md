...
- **Camera opens but nothing scans**: fixed — the scan box was previously a fixed pixel size that could exceed the actual video resolution on some phones, which silently stopped decoding. It's now sized relative to the live camera feed, and barcode formats (UPC/EAN/Code128/etc., not just QR) are explicitly enabled.
